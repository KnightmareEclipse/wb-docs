...
Entworfen sind sie — Versandschicht und Läufe in `idea/03-container-anwendung.md`, die Sofortzahlung in `api/gemeinsam.md`. Gebaut ist keiner, und fünf Punkte daran sind konkret genug, um sie hier festzuhalten — drei Schemaergänzungen, ein Lauf ohne Bauherrn und ein Secret:

- **`UNIQUE` auf `payments.payment_reference`** als eigene Migration — Anker der Idempotenz, Begründung und Grenzen in `api/gemeinsam.md`, „Sofortzahlung". Ein schlichtes `UNIQUE` genügt: Postgres lässt darin beliebig viele NULL zu, und leer bleibt die Spalte genau dort, wo die Buchhaltung von Hand bestätigt.
- **Dem Putzdienst fehlen vier Lauf-Marken, nicht eine** (`schema/putzdienst-schema.sql`). An `cleaning_cycles`: dass die Fenster-offen-Mail raus ist (Z2), dass die Zuteilung gelaufen ist (Z4) und dass die Zuteilungsmail raus ist (Z6). An `cleaning_slots`: die zwei Erinnerungen je Termin (Z9). **`allocation_released_at` ist für keine davon zu gebrauchen** — sie trägt die Freigabe durch das Sekretariat und nicht den Lauf, und ein Lauf, der sie so liest, teilt zwischen Fensterschluss und Freigabe alle fünf Minuten neu zu. Allein der Monatslauf hat seine (`cleaning_assignments.penalty_handed_over_at`). Welche Form die vier bekommen, entscheidet der Bau des jeweiligen Laufs; eine Zustandsdatei neben der Datenbank ist keine davon.
