...
Gebaut sind inzwischen die Versandschicht (`wb-backend/app/services/mail.py`) und der Lauf-Dienst mit leerem Register (`wb-backend/app/runs.py`), dazu der `UNIQUE` auf `payments.payment_reference`. Was daran noch fehlt, sind vier Punkte — drei Schemaergänzungen, ein Lauf ohne Bauherrn und ein Secret:
