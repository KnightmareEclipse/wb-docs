...
Gebaut sind inzwischen die Versandschicht (`wb-backend/app/services/mail.py`) und der Lauf-Dienst (`wb-backend/app/runs.py`), dessen Register mit der Fenster-offen-Mail seine erste Zeile trägt, dazu der `UNIQUE` auf `payments.payment_reference`. Was daran noch fehlt, sind vier Punkte — drei Schemaergänzungen, ein Lauf ohne Bauherrn und ein Secret:

- **Dem Putzdienst fehlen drei Lauf-Marken** (`schema/putzdienst-schema.sql`). An `cleaning_cycles`: dass die Zuteilung gelaufen ist (Z4) und dass die Zuteilungsmail raus ist (Z6). An `cleaning_slots`: die zwei Erinnerungen je Termin (Z9). Alle drei hängen an der Zuteilung, die es noch nicht gibt; die Fenster-offen-Mail hat ihre (`cleaning_cycles.registration_mail_sent_at`), der Monatslauf ebenso (`cleaning_assignments.penalty_handed_over_at`). **`allocation_released_at` ist für keine davon zu gebrauchen** — sie trägt die Freigabe durch das Sekretariat und nicht den Lauf, und ein Lauf, der sie so liest, teilt zwischen Fensterschluss und Freigabe alle fünf Minuten neu zu. Welche Form die drei bekommen, entscheidet der Bau des jeweiligen Laufs; eine Zustandsdatei neben der Datenbank ist keine davon.
