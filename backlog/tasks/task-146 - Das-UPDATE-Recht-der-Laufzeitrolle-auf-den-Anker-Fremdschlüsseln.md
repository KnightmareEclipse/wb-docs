...
Geschlossen als Klasse, nicht als Einzelfall: 29 GRANT-UPDATE-Listen in neun Migrationen tragen `person_id`, `child_id` oder `family_id` nicht mehr — 33 Spalten auf 28 Tabellen. Die Gegenprobe steht als vierte Bedingung in tests/test_privileges.py und leitet sich wie die drei davor aus dem Katalog ab, statt aus einer gepflegten Liste. Die Regel steht in wb-backend/CLAUDE.md §6.

Eine benannte Ausnahme: `login_sessions.person_id`. `PUT /auth/identity` verschiebt die Sitzung absichtlich auf eine andere Person, und die Migration sagt das schon in ihrem Kommentar.

Ein Test musste weichen: `test_a_document_of_a_foreign_child_is_not_found` hängte ein Dokument per UPDATE an ein anderes Kind um. Es entsteht jetzt gleich am anderen Kind — dieselbe Aussage, ohne den Griff, den die Regel verbietet.
<!-- SECTION:DESCRIPTION:END -->

## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Entschieden: Ankerschlüssel grundsätzlich ohne UPDATE, oder ausgeschriebene Zeile, dass das Recht bleibt
- [x] #2 Bei Entzug: alle betroffenen Migrationen in einem Zug, nicht die eine der Gesundheit
- [x] #3 Bei Entzug: die Bedingung steht in tests/test_privileges.py, aus dem Katalog gelesen und nicht als Spaltenliste
- [x] #4 Die Entscheidung steht als Satz in CLAUDE.md des Backends, Abschnitt 6
<!-- AC:END -->
