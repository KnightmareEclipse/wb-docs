...
3. **Löst Standardsoftware es?** Eine etablierte Standard-Datenbank statt eigener Datenhaltung, ein Standard-Reverse-Proxy statt Eigenbau, ein Standard-Backup-Tool statt Eigenbau, ein vorhandener Log-Mechanismus statt eigenem Log-Stack. Konkret gewählt: PostgreSQL, Caddy, `pg_dump`+`age` (`project-parts.md`); allein beim Log-Mechanismus folgt die endgültige Wahl noch mit dem tatsächlichen Log-Volumen.
...
- **Datensparsamkeit:** nur speichern, was ein konkreter Schulprozess tatsächlich braucht. Belegt heißt: das Feld steht auf einem realen Formular **oder** ein benannter Prozess braucht es — die bloße Existenz in einem Fremdsystem-Export (ASV-BW führt allein im Schülerstamm rund 90 Spalten) ist keine Begründung. Gilt ein Feld nur für eine Rolle, steht es an der Rollentabelle statt an einer gemeinsamen Basistabelle: dann kann es für die übrigen Rollen gar nicht erst befüllt werden, auch nicht versehentlich durch einen Import.
