...
3. **Ferienanmeldung** (Ferienprogramm, Kochwerkstatt) — Buchung durch Eltern, mehrere Kinder je Formular, gebucht wird je Tag und Betreuungsende (14 oder 16 Uhr), Bezahlung beim Absenden. Kapazität je Tag begrenzt, Anmeldeschluss dynamisch vor Programmbeginn (es muss vorab eingekauft/geplant werden). Erfasst außerdem eine Notfallnummer und, bei schulfremden Kindern, die Anschrift. Stornierungen laufen heute per Mail an den Hort, der seine Excel-Liste von Hand nachzieht. Auch für schulfremde Personen. Fällig ab spätestens Weihnachten 2026.
   *Berührung:* liest, ob ein Kind eingeschrieben ist (das Formularfeld „Clemens-Kind" ist daraus ableitbar, kein eigenes Feld). Verändert nichts. **Erzeugt** schulfremde Kinder samt Familie, anmeldendem Elternteil und Notfallkontakt — einen `payers`-Datensatz dagegen nur bei Kostenübernahme (Jugendamt): die drei Stripe-Anlässe brauchen weder IBAN noch Mandat (`domains/grenzkarte.md`, Q3), der regulär zahlende Elternteil bekommt also keinen, sonst entstünde eine Zahler-Zeile ohne einen einzigen Nutzlast-Wert. Zahlerin ist dabei die handelnde Sachbearbeiterin, die Behörde trägt `payers.account_holder` (`domains/stammdaten.md`). Drei Folgen: der Löschfrist-Anker `children.exit_date` bleibt bei diesen Kindern dauerhaft NULL, die Frist muss deshalb an der letzten Buchung hängen und über das Kind hinaus auch Elternteil, Familie und Notfallkontakt erfassen (`idea/06-dsgvo-organisatorisch.md`); der **Notfallkontakt braucht einen Namen, sobald er eine dritte Person ist** — die heutige Liste führt dort nur eine nackte Nummer, und ohne Namen scheitert seine `persons`-Zeile an `last_name NOT NULL` (der vollständige Name des anmeldenden Elternteils wird bereits erhoben); und die Werbe-Mail-Einwilligung ist eine Q1-Zustimmung mit dem Zweck „Ferienbetreuung" — widerrufbar und mit Zustelladresse, also die Querschnitts-Entität, nicht ein eigenes Feld dieser Domäne und erst recht keines an `persons`. Die Kochwerkstatt ist auch für Erwachsene offen; ein erwachsener Teilnehmer ist eine `persons`-Zeile ohne Rollenzeile.
...
- Putzdienst-eigenes Datenmodell darauf aufbauend — steht: `domains/putzdienst-schema.sql` samt Prüfskript, Fachbeschreibung `domains/putzdienst.md`. Enthält zugleich die erste gebaute Querschnitts-Entität (Zahlungsvorgang Q3)
