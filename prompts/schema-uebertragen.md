...
Gegenstück zu [`prompts/schema-bauen.md`](schema-bauen.md). Dort entsteht das SQL, hier wird daraus Code. **Eine Domäne je Durchgang** — dieselbe Portionierung wie beim Bau, und aus demselben Grund. Übertragen ist, was in `wb-backend` ein Modellmodul hat; dieser Prompt gilt der nächsten Domäne ohne eines.
...
- **Änderungen an der Schreibschicht.** Sie steht und trägt den Bestand; deine Domäne fügt sich ein oder meldet einen Fund.
