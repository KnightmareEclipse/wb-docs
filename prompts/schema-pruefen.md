...
ersetzen, absenden. Jeder Lauf schreibt in seine eigene Datei und fährt seinen eigenen Container —
deshalb ist es gleich, ob du sie nacheinander startest oder mehrere nebeneinander. Was sie sich
teilen, ist nur der Arbeitsbaum, und keiner ändert darin etwas außer seinem Bericht. Der Engpass ist
nicht die Datenbank, sondern dass jede Session denselben Vorspann liest und `xhigh` denkt.

**Der Lauf über alle kommt zuletzt** und hat seinen eigenen Abschnitt unten. Er sieht, was keine
einzelne Domäne sehen kann.

Der Lauf ist nur etwas wert, wenn er unabhängig ist: eine frische Session, die den Bau nicht
mitgemacht hat und die Begründungen in den Dateien für Behauptungen hält, nicht für Belege. **Eine
Session, eine Domäne** — nicht zwei nacheinander im selben Fenster, sonst schickt sie beim Prüfen
der zweiten die ganze erste mit und du zahlst denselben Kontext doppelt.

Effort `xhigh`. Vorher `git status` sauber **bis auf `pruefberichte/aktuell-*.md`** — die Berichte
der Läufe, die neben dir laufen, sind kein unsauberer Baum. Dein Lauf legt genau eine Datei an,
`pruefberichte/aktuell-DOMÄNE.md`, und ändert sonst nichts. Alles unter dem Strich ist der Prompt.
...
Wir prüfen die Fachdomäne **DOMÄNE** im Datenmodell unter `schema/`: `schema/DOMÄNE-schema.sql`
samt `schema/DOMÄNE-schema-check.sql`, gegen die Soll-Blöcke, aus denen sie abgeleitet ist. Ob das
gelungen ist, ist deine Frage. Du änderst nichts und baust nichts nach — auch nicht „nur eben
schnell".

Steht statt einer Domäne **`gesamt`** da, ist deiner der Lauf über alle: Dann gilt der Abschnitt
„Der Lauf über alle" unten und von den sieben Fehlerklassen allein die sechste.
...
**Zuerst der Vorspann**, den jeder Lauf liest, weil er für alle Domänen gilt:
...
4. **Die Blöcke in `soll-prozesse/`, die sie nennt** — vollständig, nicht überflogen.
...
Ein Schema samt Prüfskript und seinen Blöcken sind schnell mehr, als bequem in ein Fenster passt,
und ein langer Lauf wird unterwegs zusammengefasst. Wer alles zuerst liest und erst am Ende
...
- **Funde schreibst du sofort nach `pruefberichte/aktuell-DOMÄNE.md`**, angehängt, im Format unten
  unter einer Überschrift mit dem Namen deiner Domäne. Das ist die einzige Datei, die dieser Lauf
  anlegt, und **du fasst keine fremde an** — auch nicht lesend: Ein Bericht neben dir gehört einem
  Lauf, der unabhängig sein soll.
- **Wo du stehst, sagt deine Datei** und nicht deine Erinnerung. Fang nach einer Zusammenfassung
  bei der ersten Behauptung an, die dort noch keine Zeile hat — auch dann, wenn du meinst, sie
  schon geprüft zu haben.
...
6. **Ein Sachverhalt steht an zwei Orten.** `rules.md` Abschnitt 1. Nimm die Liste der
   ...
   Namen sind meist zwei Sachverhalte, manchmal aber einer. **Diese Klasse gehört dem Lauf über
   alle** — wer eine Domäne prüft, sieht die zweite Stelle nicht und meldet sie deshalb nicht.
...
- **Die Gegenprobe ist nur isoliert grün.** Ein Skript, das mit erfundenen Fremdschlüssel-Werten
  arbeitet, läuft durch, solange die Zieltabelle noch nicht geladen ist. Lass deins deshalb gegen
  die **vollständige** Datenbank laufen, nicht nur gegen seine eigenen Voraussetzungen — auch dann,
  wenn du nur eine Domäne prüfst.
- **Die Gegenprobe wird aus dem falschen Grund abgewiesen.** Eine Probe, die den `CHECK` belegen
  ...
  Constraint-Namen aus der Fehlermeldung; das Skript änderst du dafür nicht. Ein Skript trägt
  schnell einige Dutzend Proben — geh nicht alle durch, sondern die, deren Regel dir beim Lesen der
  Blöcke ohnehin fraglich war.
- **Eine Regel hat gar keine Gegenprobe.** Zähl die Regeln aus deinen Blöcken gegen die
  Proben im Skript. Eine Regel ohne Gegenprobe gilt als nicht gebaut — auch wenn der Constraint
  dasteht.

## Wie du läufst

Aufruf, `ON_ERROR_STOP=1` und Ladereihenfolge stehen in `CLAUDE.md`. **Der Container trägt den
Namen deines Laufs** — `wb-pruef-DOMÄNE` statt `wb-pruef`, sonst reißt der Lauf neben dir dir die
Datenbank unter den Füßen weg. Ports mappt keiner, mehr braucht es zur Trennung nicht.

Drei Schritte, in dieser Folge:

1. **Alle `schema/*-schema.sql` in eine leere Datenbank**, auch die der anderen Domänen: Deine
   Fremdschlüssel zeigen dorthin, und ein Prüfskript mit erfundenen Werten läuft grün, solange die
   Zieltabelle fehlt. `klassenbildung`, `m365` und `selfservice` legen keine Tabellen an; sie zu
   laden ist ein Nichts. Scheitert der Ladelauf **in der dokumentierten Reihenfolge**, ist das ein
   Fund und kein Bedienfehler.
2. **Dein Prüfskript gegen diese vollständige Datenbank.** In den Bericht kommt der Rückgabewert,
   nicht der Text auf dem Schirm. Die übrigen dreizehn fährt der Lauf über alle; sie hier
   mitzuzählen wäre dreizehnmal dieselbe Zahl in dreizehn Berichten.
...
Er läuft zuletzt, wenn die Domänenläufe liegen, und schreibt nach `pruefberichte/aktuell-gesamt.md`.
Er liest keine Blöcke und urteilt über keine einzelne Regel — er sieht, was keiner der anderen sehen
konnte, und arbeitet dafür mechanisch:
...
- **Die Marken `[A!]`** aus allen Domänen in je einer Zeile: Domäne, Aussage, ob ein Block sie
  entscheidet.
- **Tote Namen**: ein Tabellen-, Spalten- oder Constraint-Name, den eine `.md` nennt und keine
  `.sql` mehr trägt. Das ist der Rest, den eine Schemaänderung liegen lässt, und keine einzelne
  Domäne stolpert darüber.
...
```
[ANMELDUNG-F1] anmeldung · Klasse 2 · contracts
Block 09 sagt „…"; `uq_contracts_running` weist den zweiten Fall ab.
Vorschlag: partieller Index über … statt UNIQUE.
```

- **Jeder Fund trägt eine Kennung aus Domäne und Nummer**, `[ANMELDUNG-F1]`, `[ANMELDUNG-F2]`, …,
  durchlaufend über deinen Bericht. **Der Domänenteil ist nicht Zierde:** Läuft neben dir eine
  zweite Session, vergibt sie ebenfalls bei eins, und zwei Berichte mit je einem `[F1]` sind für
  die Reparatur nicht mehr auseinanderzuhalten. Der Lauf über alle schreibt `[GESAMT-F1]`.
  Du vergibst die Kennung beim Aufschreiben, und sie **bleibt, wenn du am Ende nach Gewicht
  sortierst**: Sie ist der Griff, an dem die Reparatur den Fund anfasst und an dem ihr Commit hängt.
  Ein `[F]` ohne Kennung ist kein Fund, den jemand später ansprechen kann.
...
einem eigenen Durchgang über `pruefberichte/aktuell-DOMÄNE.md`. Der umgekehrte Weg, schon beim Lesen zu entscheiden,
...
- **Trenn Fund von Geschmack.** Eine Tabelle, die du anders geschnitten hättest, ist weder ein Fund
  ...
  dir nicht gefällt.
- **Eine Zeile am Ende**: ob deine Domäne ohne Fund durchgekommen ist.
- **Die Sortierung nach Gewicht nennt die Kennungen** — sonst ist die Liste, aus der die Reparatur
  ihre Pakete schneidet, nicht auf die Funde zurückzuführen.

## Zwischenmeldungen

**Eine Zeile**, wenn die Domäne durch ist — Name, Zahl der Funde, Prüfskript grün oder rot. Am Ende
ist der Bericht die Datei plus höchstens zehn Zeilen Prosa drumherum; die beiden Listen zählen nicht
mit.

## Was du nicht tust

- **Nichts ändern außer `pruefberichte/aktuell-DOMÄNE.md`.** Keine `.sql`, kein Commit, kein „ich
  hab's gleich mit repariert". Auch das Prüfskript nicht, dessen Lücke du gefunden hast. Und keine
  fremde `aktuell-*.md`: Die gehört einem Lauf, der ohne dich zu seinem Urteil kommen soll.
- **`pruefberichte/aktuell-DOMÄNE.md` ist deine eigene Datei** und existiert am Anfang nicht.
  Berichte früherer Zyklen liegen nicht im Arbeitsbaum, sondern in der Git-Historie — dort bleiben
  sie auch: Ein Fund, den du unabhängig wiederfindest, wiegt schwer, und das merkst du nur, wenn du
  frisch suchst.
- **Den Marken `[A]`, `[A!]` und `[?]` nicht widersprechen.** Sie sind bewusst offen; dass sie
  ...
  ein Feld. Alle `[A!]` deiner Domäne nennst du am Ende in je einer Zeile: Aussage, ob ein Block
  sie entscheidet.
