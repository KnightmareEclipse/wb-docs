...
*   **`app-stack-repo/`** — Phase 4. Lebt im späteren App-Stack-Repo (lokal `app-stack/`) und läuft über einen Git-Push-Auslöser (Bare-Repo + `post-receive`-Hook) gegen einen eingeschränkten `deploy`-User — keine dauerhaft lokal vorgehaltenen Secrets für Routine-Deploys. Stack: FastAPI/PostgreSQL/Caddy, siehe `project-parts.md`.
...
4. [App-Stack-Deploy](app-stack-repo/04-app-stack-deploy.md) — `[Konzept + Tools fixiert, Umsetzung offen]`

Kompletter Ablauf für einen Neuaufbau von Grund auf: [Runbook](runbook.md).

## Automatisierungsgrenze

Server-Provisioning, Ersteinrichtung und jeder Reboot danach laufen komplett automatisch. Manuell bleiben nur die einmaligen Konto-Bootstrap-Schritte, die zwingend ein von einem Menschen gehaltenes Geheimnis voraussetzen — Schritte 3 (healthchecks.io), 5 (Deploy-Auslöser) und 7 (Identitätsanbieter-Registrierung) im [Runbook](runbook.md), die nur bei Neuanlage bzw. Rotation des jeweiligen Kontos/der Registrierung anfallen; Schritte 5 und 7 erst sobald das App-Stack-Repo existiert.
