...
*   **`deploy`-User:** legt einen eingeschränkten `deploy`-SSH-User an (kein Root) — die Schnittstelle, über die [Phase 5b](../app-stack-repo/05b-app-stack-deploy.md) später andockt.
    *   Der Public Key für den GitLab-CI-Deploy-Zugriff (Bootstrap siehe Runbook-Schritt 4b) wird dabei idempotent wie die übrigen Admin-Keys in die `authorized_keys` des `deploy`-Users provisioniert.
