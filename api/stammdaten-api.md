...
**Drei Domänen ohne eigene Tabellen laufen hier mit** und sind deshalb keine eigene Datei
(`grenzkarte.md`): Eltern-Selfservice (02), M365-Kontenverwaltung (13) und Klassenbildung (15).
Ihre Routen gehören der Domäne, der die Daten gehören — das ist diese.
...
einer anderen Route. Es gibt **44 Routen**; **36** nennen eine Ablaufzeile, **8** eine andere Stelle:
...
| `GET /children/{child_id}` — der volle Satz eines Kindes | [02](../soll-prozesse/02-datenaenderung.md) „Was dabei erhoben wird" | `secretariat`, `school_management`, `executive_management`, `accounting`, `day_care_management`, `day_care_staff`, `teacher` | Schulleitung nur die eigene Schulform; Hort nur die betreuten Kinder; Lehrkraft nur Name, Klasse und die Alltagsangaben (`glossar.md`) — ein externes Hortkind gehört **keiner** Schulform und ist für keine Schulleitung sichtbar (09) | liest | `backend_sensitive` für Konfession und Kirchengemeinde |
...
`[A]` **`confirmed_end_date` bleibt am Abhaken (`PUT /tasks/{sync_task_id}`) entgegennehmbar, obwohl
...
| `PUT /children/{child_id}/class` — ein Kind in eine Klasse setzen oder umsetzen; legt in derselben Transaktion die Nachzieh-Aufgaben an. `[A]` **Der Aktenordner zieht hier nicht mit**: Er entsteht mit der Vertragsfreigabe (08), und `child_file_folders` trägt keinen Anker für den Ordner einer Klasse — die Bewegung gehört der Domäne, die den Ordner anlegt. — Alternative: hier einen Klassenordner adressieren; Preis: ein zweiter Ablageanker, den das Schema nicht kennt, für einen Ordner, den heute nichts erzeugt | [15](../soll-prozesse/15-klassenbildung.md) Z2 und Z3 | `school_management`, `secretariat` | Schulleitung nur ihre eigene Schulform. Zwei Bedingungen: das Kind ist eingeschrieben (`ck_children_class_needs_entry`) und die Klasse trägt seine Schulart (`fk_children_class`, zusammengesetzt). **Ein Klassenwechsel mitten im Jahr ist dieselbe Handlung** und erzeugt dieselbe Aufgabenart — eine offene wird ersetzt, nicht verdoppelt | schreibt, `entra:` | — |
