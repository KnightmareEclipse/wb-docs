...
| `PUT /auth/identity` — als wer diese Sitzung weitermacht | [00](../soll-prozesse/00-zugang-und-portal.md) Z1 | Erziehungsberechtigte | nur ein Kandidat dieser Adresse; **Bedienführung, keine Sicherheitsgrenze** (`zugang.md`) — die Reichweite ändert sich nicht, der Aktor der Spur schon. Verschiebt die vorhandene Sitzung, statt eine zweite auszustellen. Je Familie trägt die Antwort ein **Label** aus den Vor- und Nachnamen ihrer Kinder, zum Unterscheiden im Patchwork-Fall — Bedienführung wie oben, keine eigene Berechtigung | schreibt, `guardian:` | — |
| `GET /auth/session` — was das Cookie wert ist: Kandidaten, aktuelle Person, erreichbare Familien samt Label | [00](../soll-prozesse/00-zugang-und-portal.md) Z2 | Erziehungsberechtigte | die eigene Sitzung; ohne sie sähe jedes Neuladen wie ein Abmelden aus, weil das Cookie `HttpOnly` ist | liest | — |
...
| `PUT /persons/{person_id}/address` — umziehen; ein Häkchen setzt dieselbe Anschrift für die Kinder mit, in einer Transaktion | [02](../soll-prozesse/02-datenaenderung.md) Z1 | Erziehungsberechtigte; `secretariat` (Umweg) | jede Person der eigenen Familien — auch der zweite Sorgeberechtigte (02: „im Normalfall also für alle Sorgeberechtigten"). **Ein Kind ist eine Person** — der Umzug allein des Kindes läuft über dieselbe Route mit seiner `person_id`, das Häkchen ist dann ohne Wirkung | schreibt, `guardian:` / `entra:` | — |
| `POST /persons/{person_id}/phone-numbers` — eine Nummer eintragen, samt Art, Bemerkung und ob sie tagsüber erreichbar ist | [02](../soll-prozesse/02-datenaenderung.md) Z1 | Erziehungsberechtigte; `secretariat` (Umweg) | jede Person der eigenen Familien — Kinder, der zweite Sorgeberechtigte und die eigenen Notfallkontakte | schreibt, `guardian:` / `entra:` | — |
...
| `PUT /persons/{person_id}/email` — eine neue Mailadresse eintragen; schickt den [Bestätigungscode](../soll-prozesse/hebel.md#zugang-und-anmeldecode) dorthin und **ändert `persons.email` noch nicht** | [02](../soll-prozesse/02-datenaenderung.md) Z1 | Erziehungsberechtigte; `secretariat` (Umweg) | **enger als Anschrift und Telefon:** die eigene Person und die Personen der eigenen Kinder, nicht der zweite Sorgeberechtigte — wer die Adresse einer fremden Person setzt, löst ihren Bestätigungscode am eigenen Postfach ein und übernähme damit ihren ganzen Scope, auch Familien, die die eigene Sitzung nie erreicht; bis zur Bestätigung steht die alte Adresse. **Fünf je Person und Stunde**, gezählt über `login_codes` wie im Anmeldepfad — die Route schickt eine Mail an eine frei gewählte Adresse, und der Absenderruf hängt daran. Je Person und nicht je Adresse: Der Missbrauchende ist angemeldet und erreicht nur die eigene Person und die eigenen Kinder, während die Empfängeradresse gerade das ist, was er variiert; das Sekretariat bleibt unberührt, weil seine zwanzig Familien zwanzig Personen mit je eins sind | schreibt, `guardian:` / `entra:` | — |
...
| `GET /employees/selectable` — die wählbaren Personen: Name und Rollen, keine Personalangaben | [12](../soll-prozesse/12-rechnungsfreigabe.md) Z1, [15](../soll-prozesse/15-klassenbildung.md) Z1 | jede Mitarbeiterrolle | eine eigene Route und keine Erweiterung der Zeile darüber: **wer eine Person auswählen lassen will, braucht keine Personalangaben**. Der Aufrufer nennt die verlangte Rolle (`role_code`) oder die auszuschließenden (`exclude_role_codes`) — die Route kennt keine Rolle selbst, das nennt der Aufrufer. Der Elternbonus ruft sie nicht mehr: Bestätigt wird nicht ([`elternbonus-api.md`](elternbonus-api.md)) | liest | — |
...
| `GET /classes/{class_id}/roster` — die **Klassenliste** als Druckansicht, [frisch erzeugt](../soll-prozesse/hebel.md#frisch-erzeugte-liste): die eingeschriebenen Kinder mit Notfallnummer, Abholberechtigten, den Alltagsangaben zur Gesundheit, dem Fotoeinverständnis und der Schuladresse | [15](../soll-prozesse/15-klassenbildung.md) „Dateien" | `teacher`, `secretariat`, `school_management` | unbeschränkt für Lehrkräfte — **neue Einsicht entsteht durch die Liste nicht, sie liegt nur beieinander**: den vollen Gesundheitsbestand sieht die Klassenlehrkraft am Kind und nicht hier. Ein abgegangenes Kind fällt ohne Zutun heraus | liest | — **Beim Bau richtiggestellt:** `backend_health_note` ist eine *Schreib*beschränkung auf den handlungsrelevanten Hinweis (`child_health_action_notes`, seit dem 04.09.2026 eine Zeile je Sichtkreis); die Laufzeit-Rolle liest ihn tabellenweit (`wb-backend`, `gesundheit_domain`-Migration). Die Klassenliste zeigt den für `class_lead` — „den alle unterrichtenden Personen sehen" (`grenzkarte.md`); `school` gibt es unter den sechs Sichtkreisen nicht. Die Liste braucht deshalb keine enge Rolle |
...
|---|---|---|---|
| Die beiden Mails der Vorarbeit — ans Sekretariat, wer nicht aufsteigt, an die Geschäftsführung, die Preise des neuen Jahres zu prüfen; die zweite legt zugleich ihre Aufgabe an, am Ziel `annual_prices` — das erste bei der Geschäftsführung, mit dem Schuljahr als Bezug | [04](../soll-prozesse/04-schuljahreswechsel.md) Z1 | der 1. Juli, ein festes Datum; die ans Sekretariat genau einmal, es wird nicht nachgefasst | `system:rollover` |
