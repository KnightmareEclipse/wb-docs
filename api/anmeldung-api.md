...
| `GET /contracts/{contract_id}/document` — der Vertrag als PDF: **vor der Freigabe frisch aus der eingefrorenen Fassung und den echten Daten erzeugt, danach die abgelegte Urkunde** | [08](../soll-prozesse/08-schulvertrag.md) Z3 „Lesen den Vertrag und unterschreiben ihn … Der Vertragstext liegt vollständig vorher vor" | Erziehungsberechtigte; `secretariat`, `school_management`, `day_care_management`, `executive_management` | nur Kinder der eigenen Familien, und **nur wer auch zeichnet**: Wem seine [Einsichtsstufe](../soll-prozesse/hebel.md#einsichtsstufe) das Unterschreiben genommen hat, liest ihn nicht mit. Der Entwurf wird **nirgends abgelegt** — der Zwischenstand, den die Konvertierung braucht, geht nicht in die Schülerakte und wird samt beider Papierkorbstufen geräumt (`dokumente.md`). Nach der Freigabe rendert sie nichts mehr: Die Urkunde steht, und ein zweites Erzeugen ist gesperrt | liest | — |
...
| `POST /children/{child_id}/sepa-mandates` — das Mandat ausfüllen und unterschreiben; **ein bestehendes wird nie geändert, sondern ersetzt** | [08](../soll-prozesse/08-schulvertrag.md) „Was dabei erhoben wird" | Erziehungsberechtigte; `secretariat` | nur Kinder der eigenen Familien. **Eine sorgeberechtigte Person genügt**, die Widerspruchsregel greift hier nicht — „es gilt wie in 02 die letzte Handlung". Es gilt sofort, die Schule zeichnet nicht gegen, der Vertrag darunter bleibt unberührt, und das abgelöste Mandat bleibt mit seinem Unterschriftsdatum stehen (`superseded_at`). **Legt dabei die Mandatsdatei an** und zeigt von der Zeile an auf sie (`sepa_mandates.document_id`) — „eine Unterlage, eine Datei"; ein ersetztes Mandat bekommt seine eigene, die alte bleibt stehen. Die BIC nur bei einem nicht-deutschen Konto; weicht der Kontoinhaber ab, stehen Anschrift und Mailadresse am Mandat. **Erzeugt die Optigem-Aufgabe erneut**, gleich zu welchem Zeitpunkt | schreibt, `guardian:`/`entra:` | IBAN und BIC |
...
| `POST /signature-links/{token}/signature` — das Fotoeinverständnis zeichnen | [08](../soll-prozesse/08-schulvertrag.md) „Beteiligte" | niemand, ohne Anmeldung | dieselbe Marke, **einmal einlösbar**: verbraucht ist sie, sobald die `consents`-Zeile ihre Antwort trägt. Sie schreibt drei Zeilen und sonst nichts — die Zustimmung des Kindes, ihre Unterschrift (`signatures.child_id`) und die abgelegte Datei, auf die die Zustimmung zeigt (`consents.document_id`) | schreibt, `guardian:` | — |
...
drei gilt die Mechanik der [Werte im System](../soll-prozesse/hebel.md#geld-und-fristen-im-system-alles-andere-fest)
...
| `PATCH /tuition-fees/{tuition_fee_id}`, `DELETE /…` — einen angekündigten Betrag ändern oder zurücknehmen | [`hebel.md`](../soll-prozesse/hebel.md#geld-und-fristen-im-system-alles-andere-fest) | `executive_management` | wie bei den Modulbeiträgen: nur vor seinem Gültigkeitstag | schreibt, `entra:` | — |
...
| `ix_contracts_care_open` lässt je Kind nur einen offenen Hortantrag zu | `POST /care-contracts` prüft gegen denselben Index, den sie später verletzt hätte, und führt die Familie in den vorhandenen Antrag — dieselbe Bauform wie `ix_applications_running` bei `POST /applications`; `GET /care/application-context` zeigt ihn dafür an |
...
| `sepa_mandates.superseded_at` markiert das abgelöste Mandat | `POST …/sepa-mandates` schreibt drei Zeilen in einer Transaktion — die neue, ihre Datei und den Stempel auf der alten; geändert wird nie eine |
