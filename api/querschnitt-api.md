...
| `DELETE /persons/{person_id}/consents/{purpose}` — sie widerrufen, und über denselben Weg jedes Newsletter- und Schulinformations-Thema abwählen | [10](../soll-prozesse/10-ferienprogramm.md) „Was dabei erhoben wird", [00](../soll-prozesse/00-zugang-und-portal.md) „Mails und Schreiben" | Erziehungsberechtigte; `secretariat` | die eigene Antwort. **Trägt die Kategorie `requires_family_recipient`, weist `trg_consents_family_floor` die Abwahl des Letzten ab** (`schema/querschnitt-schema.sql`) — „einer je Familie muss sie bekommen" spannt über zwei Personenzeilen und ist deshalb kein Constraint, sondern ein Trigger wie die vier Zulassungs-Trigger der Domänen; die Route übersetzt seinen Fehler in eine Meldung, statt die Regel ein zweites Mal zu führen. Scheidet der zweite Sorgeberechtigte aus, schaltet der Lösch-Lauf den Verbliebenen wieder ein — diesen Weg sieht der Trigger nicht, weil dabei niemand `consents` anfasst | schreibt, `guardian:` / `entra:` | — |
| `PATCH /photo-consent-records/{photo_consent_record_id}` — den Widerruf einer Fotoerlaubnis nachtragen, deren Kind längst gelöscht ist | [08](../soll-prozesse/08-schulvertrag.md) „Löschen" | `secretariat` | setzt allein `revoked_at`, nie einen anderen Wert: Der Bestand ist ein Nachweis und keine laufende Angabe. **Kein Elternweg** — nach dem Abgang gibt es keinen Portalzugang, der Widerruf kommt per Mail und das Sekretariat trägt ihn ein (`ck_photo_consent_records_created_by`) | schreibt, `entra:` | — |
| `GET /photo-consent-records` — den Nachweisbestand durchsuchen, für die Auskunft nach Art. 15 und für die Frage, ob ein bestimmtes ehemaliges Kind fotografiert werden durfte | [08](../soll-prozesse/08-schulvertrag.md) „Löschen" | `secretariat`, `executive_management` | **nicht** der breite Leserkreis von `GET /children/{child_id}/photo-consent`: Dort steht ein Ja/Nein zu einem Kind, das es gibt, hier ein Bestand über Menschen, die die Schule verlassen haben | liest | — |
...
| Handlung | Herkunft | Wer darf | Worauf eingeschränkt | Schreibt/liest | Enge Rolle |
|---|---|---|---|---|---|
| `GET /children/{child_id}/documents` — welche Unterlagen vorliegen, fehlen oder als nicht nötig festgestellt sind | [06](../soll-prozesse/06-anmeldetag.md) Z5, [08](../soll-prozesse/08-schulvertrag.md) Z4 | `secretariat`, `school_management`, `day_care_management`; Erziehungsberechtigte; **`teacher` nur, soweit freigeschaltet** | Eltern sehen **ihre offenen Unterlagen** (06) und nichts weiter; Schulleitung nur die eigene Schulform. **Drei Stände, nicht zwei**: angefordert ohne Datei heißt „fehlt", `not_required_at` heißt „nicht nötig", keine Zeile heißt „nie verlangt". **Die Lehrkraft sieht nur Dateien aus Kategorien mit `is_teacher_readable`** — heute sind das keine, und dann antwortet die Route ihr wie einem Unberechtigten. Nur lesend: ein Ablegen gibt es für sie nicht, auch nicht bei freigeschalteter Kategorie ([grenzkarte.md](../grenzkarte.md), Q2) | liest | — |
| `POST /children/{child_id}/documents` — eine Unterlage anfordern | [06](../soll-prozesse/06-anmeldetag.md) Z5 | `secretariat`, `school_management` | unbeschränkt. Welche Unterlagen ein Ziel verlangt, **folgt aus dem Ziel und wird von niemandem gepflegt** — die Liste gehört der Anmeldung, diese Route legt die einzelne Zeile an. **Sie nimmt Kategorie und Bezeichnung entgegen, nicht die Bibliothek**: Der Ordner folgt aus Kind und Kategorie und wird angelegt, wo er fehlt (`child_file_folders`); die **Art ist freiwillig**, die **Bezeichnung Pflicht** (`grenzkarte.md` Q2). **Sie legt immer eine neue Zeile an und ersetzt nie eine**: `documents` ist bewusst nicht eindeutig je Kind und Typ, und keine Regel im Schema sagt, welche Zeile gemeint wäre — der Fund unter „Am Schema aufgefallen“ ist damit entschieden. — Alternative: die jüngste offene Zeile desselben Typs überschreiben; Preis: zwei angeforderte Atteste wären eines, und das Nachreichen des ersten löschte die Anforderung des zweiten. **Den Fehlgriff nimmt die Zeile darunter zurück** | schreibt, `entra:` | — |
| `PUT /documents/{document_id}` — die Ablage festhalten, die abgelegte Datei **ersetzen** oder die Unterlage als nicht nötig feststellen. **Sie ist auch der Änderungsweg**: Ein zweites Mal eingehendes Papier — ein berichtigtes Attest, eine neu eingescannte Seite — behält seine Zeile und bekommt hier die neue Graph-Kennung, `filed_at` wird neu gestempelt. Eine bereits abgelegte Unterlage lässt sich **nicht** als nicht nötig feststellen: `ck_documents_not_required` verböte es ohnehin, die Route weist es mit `400` ab statt mit einer abgebrochenen Transaktion. **Sie nimmt die Graph-Kennung entgegen, nicht die Datei**: Das Papier legt ein Mensch in die Bibliothek, die er ohnehin offen hat — „SharePoint ist keine Aufgabe“ (08) —, und diese Zeile hält fest, wo es gelandet ist. — Alternative: ein Upload durch das Backend; Preis: ein zweiter schreibender Graph-Weg samt Größengrenze, für eine Datei, die ohnehin in einem Ordner landet | [06](../soll-prozesse/06-anmeldetag.md) Z5, [08](../soll-prozesse/08-schulvertrag.md) Z4 | `secretariat`, `school_management` | „was vorliegt, war nötig" (`ck_documents_not_required`); abgelegt heißt mit Zeitpunkt (`ck_documents_filed`). Die Zeile hängt am **Ordner** und nicht an der Bibliothek, und `fk_documents_folder` bindet sie an einen Ordner **desselben** Kindes — eine Datei in fremder Akte weist die Datenbank ab, nicht erst die Route. Die Kennung ist **nie ein Pfad** — sie überlebt Umbenennen und Verschieben | schreibt, `entra:` | — |
...
| `PATCH /configured-values/{configured_value_id}` — einen angekündigten Wert ändern | [`hebel.md`](../soll-prozesse/hebel.md#geld-und-fristen-im-system-alles-andere-fest) | `executive_management` | **nur solange `valid_from` in der Zukunft liegt** | schreibt, `entra:` | — |
| `DELETE /configured-values/{configured_value_id}` — einen angekündigten Wert zurücknehmen | [`hebel.md`](../soll-prozesse/hebel.md#geld-und-fristen-im-system-alles-andere-fest) | `executive_management` | wie oben; ein bereits gültiger bleibt stehen, denn was berechnet oder bezahlt ist, bleibt bei seinem Betrag | schreibt, `entra:` | — |
| `GET /contract-texts` — die Vertragstexte, je Code die geltende und die angekündigte Fassung | [`hebel.md`](../soll-prozesse/hebel.md#geld-und-fristen-im-system-alles-andere-fest) | `executive_management`, `secretariat`, `school_management`, `day_care_management`; Erziehungsberechtigte | Die Eltern sehen den Text, den sie unterschreiben bzw. dem sie zustimmen — **die Fassung friert mit der Zusage ein** und steht danach am Vorgang (`contracts.contract_text_id`), nicht an dieser Route | liest | — |
| `POST /contract-texts` — eine neue Fassung einfrieren: die Word-Datei aus der Arbeitsfassung holen, `body` daraus auslesen, Prüfsumme rechnen, Gültigkeitstag setzen. **Sie sagt zugleich, ob die Fassung eine wesentliche Änderung ist** (`requires_consent`) — davon hängt ab, ob den laufenden Verträgen die Kenntnisnahme genügt oder ein Nachtrag mit Zustimmung nötig wird ([08](../soll-prozesse/08-schulvertrag.md)) | [`hebel.md`](../soll-prozesse/hebel.md#geld-und-fristen-im-system-alles-andere-fest), [08](../soll-prozesse/08-schulvertrag.md) „Dateien" | `executive_management` | **allein die Geschäftsführung** — „sie verantwortet die Verwaltung und besonders die Verträge" (`glossar.md`). Je Code und Tag eine Fassung (`uq_contract_texts`) | schreibt, `entra:` | — |
| `PATCH /contract-texts/{contract_text_id}` — eine angekündigte Fassung ändern, auch erneut einfrieren | [`hebel.md`](../soll-prozesse/hebel.md#geld-und-fristen-im-system-alles-andere-fest) | `executive_management` | nur solange `valid_from` in der Zukunft liegt. **Für `template_docx` schreibt die Änderungsspur die Prüfsumme statt des Werts** — die einzige solche Ausnahme, und `ck_change_log_template` weist den Versuch ab, die Dateibytes abzulegen | schreibt, `entra:` | — |
| `DELETE /contract-texts/{contract_text_id}` — eine angekündigte Fassung zurücknehmen | [`hebel.md`](../soll-prozesse/hebel.md#geld-und-fristen-im-system-alles-andere-fest) | `executive_management` | wie oben | schreibt, `entra:` | — |
...
| `consents.document_id` ist nullable | Gefüllt, wo eine Unterschrift entsteht: Der Signaturlink des Kindes ab 14 erzeugt die Datei und lässt die Zustimmung auf sie zeigen ([`anmeldung-api.md`](anmeldung-api.md)). Die Antwort der Eltern über diese Route trägt keine Unterschrift und bleibt eine Zeile ohne Datei |
| `consents.child_id` kaskadiert, `person_id` **hält fest** | Die Zustimmung am Kind hat nach dem Kind keinen Gegenstand mehr und geht mit ihm; die kindlose räumt der Lösch-Lauf selbst und lässt genau die stehen, die abbestellbar und nicht widerrufen ist ([17](../soll-prozesse/17-loesch-lauf.md)). Die Route berührt das nicht, aber `GET /children/{child_id}/consents` darf keine Zeile eines gelöschten Elternteils erwarten |
| `photo_consent_records` hat keinen Fremdschlüssel auf Kind oder Person | Der Bestand entsteht erst, wenn beide fort sind. Keine Route legt ihn an — das tut allein der Lösch-Lauf, und `created_by` lässt kein `guardian:` zu |
