...
- Familie ist die Sorgerecht-Konstellation aus `domains/stammdaten.md`, **vom Sekretariat manuell gepflegt** und nicht algorithmisch hergeleitet. Der Schulvertrag knüpft die Pflicht an dieselbe Einheit: „Personensorgeberechtigte, die mehrere Schüler/innen beim Schulträger angemeldet haben, erbringen die Mitarbeit einmal pro Familie."
...
- **Pflichtig ist eine Familie, sobald sie mindestens ein eingeschriebenes Kind hat.** Eingeschrieben heißt (`domains/stammdaten.md`): `exit_date IS NULL` **und** entweder eine aktive Klasse (`classes.is_active`) oder eine gesetzte `provisional_grade_level_id`. Beide Hälften sind nötig — ein Abgänger behält seine `class_id`, und ein neu aufgenommenes Kind hat vor der Klassenzuteilung nur die vorläufige Klassenstufe. Ein reines Ferienprogramm-Kind (`fachdomaenen.md` Abschnitt 1) erfüllt keine der beiden und löst damit keine Pflicht aus.
...
- **Amts- und Vereinsvormundschaft sind befreit, aber nicht weil sie keine Personen wären.** Eine Amtsvormundin ist im Schema eine ganz normale Erziehungsberechtigte (`domains/stammdaten.md`, „Familie") und hätte damit auch einen Buchungszugang. Dass sie nicht putzdienstpflichtig ist, trägt das Kennzeichen `exempt_from_parent_duties` an ihrer Kategorie in `family_guardians` — eine ausdrückliche Aussage über diese Erziehungsberechtigung statt eines Nebeneffekts der Rechtsform. Fehlt die Kategorie, gilt pflichtig; Pflegeeltern sind ausdrücklich pflichtig.
...
Befreit ist, wer **aktuell** beschäftigt ist: eine `employees`-Zeile zu dieser Person mit leerem `employment_end` (`domains/stammdaten-schema.sql`). Kein Flag am Erziehungsberechtigten — das wäre ein zweiter Ort für dieselbe Tatsache und träfe zudem die falsche Aussage, weil ein ausgeschiedener Mitarbeiter wieder pflichtig wird. Die Tabelle ist generisch und nicht Teil des Putzdienst-Schemas.
...
- Konkrete Putztermine: Datum, Terminart, Stundenzahl (trägt den Stundennachweis, siehe „Prozess") — Kapazität wird nicht hier eingetragen, sondern in zwei Stufen berechnet (siehe „Restplatz-Zuordnung")
