...
Domäne 2/4 aus `fachdomaenen.md` Abschnitt 6 — **eine** Domäne in drei Phasen, weil dieselbe Bewerbung sie alle durchläuft. Tabellenschema: `domains/anmeldung-schema.sql`, belegt durch `domains/anmeldung-schema-check.sql` (Sollstand 60/60). Der heutige Ablauf samt Formularfeldern steht in `prozesse.md` Abschnitt 3–7; hier steht, was daraus im Datenmodell folgt.
...
- **Das Signaturbild ist ebenfalls eine Datei dieser Bibliothek** und trägt deshalb dieselbe Graph-Kennung wie ein Dokument, keinen Pfad. **Es wird mit dem Abschluss des Vorgangs gelöscht** (`contracts.confirmation_sent_at`): dann steckt es im erzeugten PDF, und daneben wäre es nur eine zweite Kopie desselben Personendatums ohne Abnehmer. Bis dahin wird es gebraucht — die Korrektur vor Abschluss erzeugt das Dokument neu und setzt die geleisteten Unterschriften wieder ein (siehe unten). Eine Signaturzeile ohne Bild heißt damit „unterschrieben, Bild abgeräumt", nie „hat nicht unterschrieben"; so kommt auch der Altbestand herein, für den es nie ein Bild gab.
