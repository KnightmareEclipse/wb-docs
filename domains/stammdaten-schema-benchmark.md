...
- **Das Standard-`/dev/shm` von Containern (64 MB) reicht nicht** für Postgres' parallele Worker unter gleichzeitiger Last — führt zu `could not resize shared memory segment`-Fehlern ab ~20 parallelen Verbindungen. Gilt für Docker wie Podman gleichermaßen. Kein Schema-Problem, sondern eine Startparameter-Frage. **Für `wb-backend/docker-compose.yml` zu übernehmen:** `shm_size` explizit setzen (in diesem Test mit 1024 MB stabil, kleinere Werte nicht systematisch ausgetestet).
