...
- Die Rollen stehen (`glossar.md`): den vollen Satz sehen Verwaltung, Klassenlehrer:in und Hort, den Hinweis formuliert die Klassenlehrkraft. Offen ist allein ihre Umsetzung in `init-roles.sh` — schreibend ist der Hinweis enger als lesend, `UPDATE (action_note)` bekommt nur die Klassenlehrer:in (`TODO.md`).
