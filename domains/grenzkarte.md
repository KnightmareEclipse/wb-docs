...
Das trifft genau drei Anlässe, alle über **Stripe**: Voranmeldung samt Quereinstieg, Ferienprogramm-Buchung und Putzdienst-Freikauf. Form entsprechend schmal: *Anlass (Domäne + Vorgang) × Betrag × Status × Zahlungsreferenz*. Der Putzdienst-Freikauf zeigt dabei auf **zwei** Vorgänge, nicht auf einen: den Komplett-Freikauf der Jahrespflicht (Familie × Zyklus) und den Einzel-Freikauf eines bereits zugeteilten Termins (`putzdienst.md`, „Freikauf & Zahlung"). Das ändert nichts an der Bauform — es ist eine weitere Vorgangs-Spalte im selben Entweder-oder-CHECK. Status bleibt trotzdem zahlungswegneutral (offen/bestätigt): neben Stripe bleibt die manuelle Bestätigung durch die Buchhaltung als benannter Ausweg für Überweisung und Bargeld bestehen (`putzdienst.md`) — dieselbe Zeile trägt beides, die Zahlungsreferenz bleibt dann leer.

**Gebaut ist Q3 mit den beiden Putzdienst-Freikäufen** (`putzdienst-schema.sql`): Komplett-Freikauf und Einzel-Freikauf hängen als je eine nullable Vorgangs-Spalte an `payments`, zusammengehalten von einem CHECK, der genau einen Anlass je Zahlung erzwingt. Voranmeldung und Ferienprogramm-Buchung kommen genauso dazu — je eine weitere Spalte, ein Summand mehr im CHECK, keine zweite Zahlungstabelle: der Fremdschlüssel trägt die Unterscheidung, wie bei `guardians` und `payers` in Stammdaten. Ein Typ-Feld plus untypisierte Vorgangs-ID gäbe die referenzielle Integrität auf, und genau die ist der Grund, Q3 überhaupt einmal zu bauen.
...
- **`payers` in Stammdaten trägt das Einzugs*mittel*** (die Bankverbindung; das Mandat dazu steht je Kind an `children`) und ist von Q3 unabhängig: die drei Stripe-Anlässe brauchen weder IBAN noch Mandat, und das Mandat trägt keine Zahlung. Folge für die Ferienanmeldung: der regulär über Stripe zahlende Elternteil bekommt **keine** `payers`-Zeile — sie hätte keinen einzigen Nutzlast-Wert. Eine entsteht dort nur bei Kostenübernahme durch das Jugendamt (`stammdaten.md`, „Zahlungsverantwortliche").
...
**Der Beschäftigungszeitraum ist beidseitig nötig, nicht nur das Ende.** Eine `employees`-Zeile entsteht regelmäßig vor dem ersten Arbeitstag, weil die M365-Konten des neuen Schuljahres Ende Juli angelegt werden — ohne `employment_start` gälte die Person ab dem Anlegen als beschäftigt und bekäme Lehrkraft-Zugriff samt Putzdienst-Befreiung Wochen zu früh; eine früh eingetragene Kündigung entzöge beides spiegelverkehrt zu früh. „Aktuell beschäftigt" ist deshalb das vollständige Zeitraum-Prädikat (`stammdaten-schema.sql`, `employees`) und darf nirgends auf „`employment_end IS NULL`" verkürzt werden. Weitere Personalfelder (Funktion, Stundenumfang) gibt es nicht: die Personaldatenquelle der Schule existiert, wird aber bewusst **nicht** nach Weltenbaum übertragen — kommt sie später, ist jedes Feld daraus eine nullable Spalte und vom Freeze nicht berührt.
