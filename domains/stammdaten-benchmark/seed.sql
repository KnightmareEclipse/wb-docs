...
CREATE TABLE bench_pool_children AS
    SELECT row_number() OVER () AS n, id AS child_id FROM (
        SELECT child_id AS id FROM children ORDER BY random() LIMIT 1000
    ) s;
CREATE TABLE bench_pool_families AS
    SELECT row_number() OVER () AS n, id AS family_id FROM (
        SELECT family_id AS id FROM families ORDER BY random() LIMIT 1000
    ) s;
CREATE TABLE bench_pool_classes AS
    SELECT row_number() OVER () AS n, id AS class_id FROM (
        SELECT class_id AS id FROM classes ORDER BY random() LIMIT 1000
    ) s;
CREATE TABLE bench_pool_guardian_persons AS
    SELECT row_number() OVER () AS n, p.person_id FROM (
        SELECT g.guardian_id AS id FROM guardians g ORDER BY random() LIMIT 1000
    ) s JOIN persons p ON p.person_id = s.id;
...
CREATE TABLE bench_pool_phones AS
    SELECT row_number() OVER () AS n, id FROM (
        SELECT phone_number_id AS id FROM phone_numbers ORDER BY random() LIMIT 1000
    ) s;
