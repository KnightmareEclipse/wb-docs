...
**Sie hält als Einzige die Bankverbindung** (`sepa_mandates.iban`/`bic`) — eigene, engere DB-Rolle mit Spalten-GRANT wie bei den Art.-9-Spalten, nicht Teil der pauschalen Laufzeit-Rolle (`schema/stammdaten-schema.sql`; `TODO.md`). Sie ist der benannte Abnehmer dieser Spalten: die Bankverbindung wandert einmal von Hand nach Optigem, sobald die Verträge samt Mandat vorliegen (`fachdomaenen.md` Abschnitt 3). Das SEPA-Mandat selbst (`children.mandate_reference`/`mandate_signed_at`) braucht dafür keinen eigenen GRANT — es steht am Kind, und die Kinder sieht sie ohnehin.
