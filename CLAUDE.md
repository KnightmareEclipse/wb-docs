...
Konzept- und Architektur-Doku für einen selbstverwalteten, DSGVO-konformen Datenbank-/API-VPS (Hetzner) für Schulprozesse — LUKS-Verschlüsselung mit manuellem Unlock, Docker-Netzwerk-Isolation, OIDC/Entra-ID-Auth, Restic-Backups. Details in `idea.md`, `pipeline.md`, `project-parts.md`. Die Planungsprinzipien, denen alle drei folgen, stehen in `rules.md` — jede neue Entscheidung in diesem Repo hält sich daran.
