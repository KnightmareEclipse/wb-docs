...
Das VPS-Repo (`pipeline/`) ist in Phase 1–2 fertig automatisiert (Provisioning, Firewall/SSH-Härtung). Phase 3 (Rootless Podman) ist entschieden und auf dem Host erprobt, aber noch nicht als Skript/Rolle umgesetzt. Zwei offene Umbauten hängen daran: die Host-Konfiguration liegt weiterhin als Shell-Skripte in `wb-vps/setup/` statt als die in `rules.md` Abschnitt 1 beschlossenen Ansible-Rollen, und die Podman-Rolle sollte gleich als erste Ansible-Rolle entstehen statt zweimal geschrieben zu werden.
...
Danach **einmal** validieren, nicht nach jedem Einzelpunkt: alle fünf Prüfskripte in Ladereihenfolge — Stammdaten (65/65), Putzdienst (22/22), Anmeldung (37/37), Ferien (11/11), Gesundheit (9/9); Aufruf und Sollstand stehen im jeweiligen Kopfkommentar. Bei Spaltenänderungen an Stammdaten zusätzlich der Benchmark-Generator mit `n_children=500`/`n_classes=20` — die Zeilenzahlen müssen denen aus `domains/stammdaten-schema-benchmark.md` (Durchlauf 1) entsprechen.
