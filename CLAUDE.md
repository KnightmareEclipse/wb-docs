...
Das VPS-Repo (Phasen 1–3 in `pipeline/`: Provisioning, Firewall/SSH-Härtung, Rootless Podman) ist fertig entwickelt und automatisiert. Phase 1 läuft weiter über das `hcloud`-CLI in `wb-vps/infra/`, Phasen 2–3 als Ansible-Rollen (`hardening`, `podman_rootless`) in einem `site.yml`-Lauf.
